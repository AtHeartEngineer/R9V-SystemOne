...
constexpr int kMaxLogicalCacheSlots = 48;
...
__global__ __launch_bounds__(256) void plan_expert_cache_lru_fill(
  ...
  if (routes <= 0 || routes > 64 || cache_slots <= 0 || cache_slots > 48) {
    if (threadIdx.x == 0) pending[0] = 0;
    return;
  }
  __shared__ int ids[64], tags[48], old_rank[48], last[48], rank[48];
...
// Batched synchronous planner.  Each seven-int record is independent and is
// consumed by the existing copy/publish kernels in sequence on this stream.
// Victims are never selected when their expert occurs anywhere in this route
// set; this deliberately leaves such routes on the exact cold/UVA path.
__global__ void plan_expert_cache_lru_fill_many(
    const int* __restrict__ hot_map, const int* __restrict__ cold_map,
    const int* __restrict__ expert_ids, const int* __restrict__ cache_map,
    const int* __restrict__ cache_tags, int* __restrict__ cache_clock,
    int* __restrict__ stats, int* __restrict__ pending,
    int num_experts, int cold_count, int cache_slots, int routes, int max_fills) {
  if (threadIdx.x != 0) return;
  if (routes <= 0 || routes > 64 || max_fills < 1 || max_fills > 4) return;
  for (int f = 0; f < max_fills; ++f) pending[7 * f] = 0;
  stats[0] += 1;
  for (int route = 0; route < routes; ++route) {
    const int e = expert_ids[route];
    if (e < 0 || e >= num_experts) continue;
    const int s = cache_map[e];
    if (s >= 0 && s < cache_slots && cache_tags[s] == e) {
      stats[2] += 1;
      touch_lru_slot(cache_tags, cache_clock, cache_slots, s);
    }
  }
  int chosen_e[4] = {-1, -1, -1, -1};
  int chosen_s[4] = {-1, -1, -1, -1};
  for (int f = 0; f < max_fills; ++f) {
    int best = -1, best_count = 0, best_route = routes;
    for (int route = 0; route < routes; ++route) {
      const int e = expert_ids[route];
      if (e < 0 || e >= num_experts || hot_map[e] >= 0 ||
          cold_map[e] < 0 || cold_map[e] >= cold_count) continue;
      const int cs = cache_map[e];
      if (cs >= 0 && cs < cache_slots && cache_tags[cs] == e) continue;
      bool already = false, first = true; int count = 0;
      for (int p = 0; p < f; ++p) if (chosen_e[p] == e) already = true;
      for (int r = 0; r < routes; ++r) { if (expert_ids[r] == e) { ++count; if (r < route) first = false; } }
      if (already || !first) continue;
      if (count > best_count || (count == best_count && route < best_route))
        best = e, best_count = count, best_route = route;
    }
    if (best < 0) break;
    int target = -1;
    for (int s = 0; s < cache_slots; ++s) {
      bool reserved = false; for (int p = 0; p < f; ++p) if (chosen_s[p] == s) reserved = true;
      if (!reserved && cache_tags[s] < 0) { target = s; break; }
    }
    if (target < 0) {
      int oldest = cache_slots + 1;
      for (int s = 0; s < cache_slots; ++s) {
        bool reserved = false, protected_route = false;
        for (int p = 0; p < f; ++p) if (chosen_s[p] == s) reserved = true;
        for (int r = 0; r < routes; ++r) if (expert_ids[r] == cache_tags[s]) protected_route = true;
        if (!reserved && !protected_route && cache_clock[s + 1] < oldest)
          oldest = cache_clock[s + 1], target = s;
      }
    }
    if (target < 0) break;
    chosen_e[f] = best; chosen_s[f] = target;
    int* p = pending + 7 * f;
    p[1] = best; p[2] = cold_map[best]; p[3] = target; p[4] = target;
    p[5] = cache_tags[target]; p[6] = 3; p[0] = 1;
    stats[7] += 1; stats[8] += best_count;
  }
}
...
template <int group_size, int intermediate>
bool try_launch_tiered_prefill_grouped_width(
    const torch::Tensor& cold_weight, const torch::Tensor& hot_weight,
    const torch::Tensor& hot_map, const torch::Tensor& cold_map,
    const torch::Tensor* cache_weight, const torch::Tensor* cache_map,
    const torch::Tensor& quantized, torch::Tensor& output,
  ...
  if (ncols == 2560 && nrows == 2 * intermediate) {
    if (qtype == 12) {
      launch_tiered_prefill_grouped<group_size, QI4_K / VDR_Q4_K_Q8_1_MMVQ, QK_K, 2560, 2 * intermediate,
                                   block_q4_K, VDR_Q4_K_Q8_1_MMVQ,
                                   vec_dot_q4_K_q8_1>(cold_weight, hot_weight,
          hot_map, cold_map, cache_weight, cache_map, quantized, output,
          sorted_route_ids, block_expert_ids, num_routes_post_padded, top_k,
          tokens, stream);
      return true;
    }
    if (qtype == 13) {
      launch_tiered_prefill_grouped<group_size, QI5_K / VDR_Q5_K_Q8_1_MMVQ, QK_K, 2560, 2 * intermediate,
                                   block_q5_K, VDR_Q5_K_Q8_1_MMVQ,
                                   vec_dot_q5_K_q8_1>(cold_weight, hot_weight,
          hot_map, cold_map, cache_weight, cache_map, quantized, output,
          sorted_route_ids, block_expert_ids, num_routes_post_padded, top_k,
          tokens, stream);
      return true;
    ...
    if (qtype == 21) {
      launch_tiered_prefill_grouped<group_size, 8, QK_K, 2560, 2 * intermediate,
    ...
    if (qtype == 23) {
      launch_tiered_prefill_grouped<group_size, 8, QK_K, 2560, 2 * intermediate,
                                   block_iq4_xs, 1, vec_dot_iq4_xs_q8_1>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, sorted_route_ids, block_expert_ids,
          num_routes_post_padded, top_k, tokens, stream);
      return true;
  ...
  if (nrows == 2560 && ncols == intermediate) {
    if (qtype == 7) {
      launch_tiered_prefill_grouped<group_size, 2, QK5_1, intermediate, 2560,
                                   block_q5_1, VDR_Q5_1_Q8_1_MMVQ,
                                   vec_dot_q5_1_q8_1>(cold_weight, hot_weight,
          hot_map, cold_map, cache_weight, cache_map, quantized, output,
          sorted_route_ids, block_expert_ids, num_routes_post_padded, top_k,
          tokens, stream);
      return true;
    ...
    if (qtype == 20) {
      launch_tiered_prefill_grouped<group_size, 2, QK4_NL, intermediate, 2560,
                                   block_iq4_nl, VDR_Q4_0_Q8_1_MMVQ, vec_dot_iq4_nl_q8_1>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, sorted_route_ids, block_expert_ids,
          num_routes_post_padded, top_k, tokens, stream);
      return true;
    ...
    if (qtype == 8) {
      launch_tiered_prefill_grouped<group_size, 4, QK8_0, intermediate, 2560,
                                   block_q8_0, VDR_Q8_0_Q8_1_MMVQ, vec_dot_q8_0_q8_1>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, sorted_route_ids, block_expert_ids,
          num_routes_post_padded, top_k, tokens, stream);
      return true;
...
bool try_launch_tiered_prefill_grouped(
    ...
    const torch::Tensor& num_routes_post_padded, int top_k, int qtype,
    int ncols, int nrows, int tokens, hipStream_t stream) {
  switch (ncols == 2560 ? nrows / 2 : ncols) {
    case 192:
      return try_launch_tiered_prefill_grouped_width<group_size, 192>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, sorted_route_ids, block_expert_ids,
          num_routes_post_padded, top_k, qtype, ncols, nrows, tokens, stream);
    case 224:
      return try_launch_tiered_prefill_grouped_width<group_size, 224>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, sorted_route_ids, block_expert_ids,
          num_routes_post_padded, top_k, qtype, ncols, nrows, tokens, stream);
    case 416:
      return try_launch_tiered_prefill_grouped_width<group_size, 416>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, sorted_route_ids, block_expert_ids,
          num_routes_post_padded, top_k, qtype, ncols, nrows, tokens, stream);
    case 320:
      return try_launch_tiered_prefill_grouped_width<group_size, 320>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, sorted_route_ids, block_expert_ids,
          num_routes_post_padded, top_k, qtype, ncols, nrows, tokens, stream);
    case 448:
      return try_launch_tiered_prefill_grouped_width<group_size, 448>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, sorted_route_ids, block_expert_ids,
          num_routes_post_padded, top_k, qtype, ncols, nrows, tokens, stream);
    case 640:
      return try_launch_tiered_prefill_grouped_width<group_size, 640>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, sorted_route_ids, block_expert_ids,
          num_routes_post_padded, top_k, qtype, ncols, nrows, tokens, stream);
    default:
...
template <int reuse_matches, int lanes_per_row, int qk, int ncols, int nrows,
          typename block_q_t, int vdr, vec_dot_q_cuda_t vec_dot>
__global__ __launch_bounds__(256) void tiered_moe_vec_exact_reuse3v2(
  ...
  __shared__ const block_q_t* selected_weight;
  __shared__ int selected_groups[reuse_matches];
  __shared__ int selected_count;
  __shared__ int selected_valid;
  ...
  if (threadIdx.x == 0) {
    ...
    if (expert < 0 || expert >= num_experts) {
      ...
      int first_group = (reuse_matches * 10);
      int occurrences = 0;
      int matches[reuse_matches] = {};
#pragma unroll
      for (int route = 0; route < (reuse_matches * 10); ++route) {
        if (expert_ids[route] == expert) {
          if (occurrences < reuse_matches) matches[occurrences] = route;
          if (route < first_group) first_group = route;
          ++occurrences;
        }
      }
      if (occurrences > reuse_matches) {
        selected_groups[0] = group;
        selected_count = 1;
      } else if (group == first_group) {
        selected_count = occurrences;
#pragma unroll
        for (int match = 0; match < reuse_matches; ++match) {
          selected_groups[match] = matches[match];
        }
      }
  ...
  const int row = blockIdx.x * rows_per_block + wave * rows_per_wave +
                  row_in_wave;
  float sums[reuse_matches] = {};

  const block_q8_1* activations[reuse_matches] = {};
#pragma unroll
  for (int match = 0; match < reuse_matches; ++match) {
    if (match < selected_count) {
      const int token = selected_groups[match] / top_k;
...
#pragma unroll
    for (int block = 0; block < blocks_per_row; ++block) {
      const block_q8_1* block_activations[reuse_matches];
#pragma unroll
      for (int match = 0; match < reuse_matches; ++match) {
        block_activations[match] =
            match < selected_count
      ...
      accumulate_reused_block<reuse_matches, block_q_t, vdr, vec_dot>(
...
void launch_tiered_reuse3v2(
  ...
  const dim3 grid((nrows + rows_per_block - 1) / rows_per_block,
                  (reuse_matches * 10), 1);
  tiered_moe_vec_exact_reuse3v2<reuse_matches, lanes_per_row, qk, ncols, nrows, block_q_t,
                                 vdr, vec_dot>
      <<<grid, 256, 0, stream>>>(
          cold_weight.data_ptr(), hot_weight.data_ptr(),
          cache_weight == nullptr ? nullptr : cache_weight->data_ptr(),
...
template <int reuse_matches, int intermediate>
bool try_launch_tiered_reuse3v2_width(
    const torch::Tensor& cold_weight, const torch::Tensor& hot_weight,
    const torch::Tensor& hot_map, const torch::Tensor& cold_map,
    const torch::Tensor* cache_weight, const torch::Tensor* cache_map,
    const torch::Tensor& quantized, torch::Tensor& output,
    const torch::Tensor& expert_ids, int top_k, int qtype, int ncols,
    int nrows, int tokens, hipStream_t stream) {
  if (tokens * top_k != (reuse_matches * 10)) return false;
  if (ncols == 2560 && nrows == 2 * intermediate) {
    if (qtype == 21) {
      launch_tiered_reuse3v2<reuse_matches, 8, QK_K, 2560, 2 * intermediate, block_iq3_s,
    ...
    if (qtype == 23) {
      launch_tiered_reuse3v2<reuse_matches, 8, QK_K, 2560, 2 * intermediate, block_iq4_xs,
  ...
  if (nrows == 2560 && ncols == intermediate) {
    if (qtype == 20) {
      launch_tiered_reuse3v2<reuse_matches, 2, QK4_NL, intermediate, 2560, block_iq4_nl,
    ...
    if (qtype == 8) {
      launch_tiered_reuse3v2<reuse_matches, 4, QK8_0, intermediate, 2560, block_q8_0,
                           VDR_Q8_0_Q8_1_MMVQ, vec_dot_q8_0_q8_1>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, expert_ids, top_k, stream);
      return true;
...
template <int reuse_matches>
bool try_launch_tiered_reuse_v2_rows(
    const torch::Tensor& cold_weight, const torch::Tensor& hot_weight,
    const torch::Tensor& hot_map, const torch::Tensor& cold_map,
    const torch::Tensor* cache_weight, const torch::Tensor* cache_map,
    const torch::Tensor& quantized, torch::Tensor& output,
    const torch::Tensor& expert_ids, int top_k, int qtype, int ncols,
    int nrows, int tokens, hipStream_t stream) {
  // The output remains hidden-size on both ranks; only intermediate slices differ.
  switch (ncols == 2560 ? nrows / 2 : ncols) {
    case 192:
      return try_launch_tiered_reuse3v2_width<reuse_matches, 192>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, expert_ids, top_k, qtype, ncols, nrows, tokens, stream);
    case 224:
      return try_launch_tiered_reuse3v2_width<reuse_matches, 224>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, expert_ids, top_k, qtype, ncols, nrows, tokens, stream);
    case 416:
      return try_launch_tiered_reuse3v2_width<reuse_matches, 416>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, expert_ids, top_k, qtype, ncols, nrows, tokens, stream);
    case 320:
      return try_launch_tiered_reuse3v2_width<reuse_matches, 320>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, expert_ids, top_k, qtype, ncols, nrows, tokens, stream);
    case 448:
      return try_launch_tiered_reuse3v2_width<reuse_matches, 448>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, expert_ids, top_k, qtype, ncols, nrows, tokens, stream);
    case 640:
      return try_launch_tiered_reuse3v2_width<reuse_matches, 640>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, expert_ids, top_k, qtype, ncols, nrows, tokens, stream);
    default:
      return false;
...
bool try_launch_tiered_reuse3v2(
    ...
    int nrows, int tokens, hipStream_t stream) {
  switch (tokens * top_k) {
    case 20: return try_launch_tiered_reuse_v2_rows<2>(cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map, quantized, output, expert_ids, top_k, qtype, ncols, nrows, tokens, stream);
    case 30: return try_launch_tiered_reuse_v2_rows<3>(cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map, quantized, output, expert_ids, top_k, qtype, ncols, nrows, tokens, stream);
    case 40: return try_launch_tiered_reuse_v2_rows<4>(cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map, quantized, output, expert_ids, top_k, qtype, ncols, nrows, tokens, stream);
    case 50: return try_launch_tiered_reuse_v2_rows<5>(cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map, quantized, output, expert_ids, top_k, qtype, ncols, nrows, tokens, stream);
...
torch::Tensor tiered_iq_moe_prefill_grouped_impl(
  ...
  TORCH_CHECK(qtype == 7 || qtype == 8 || qtype == 12 || qtype == 13 ||
                  qtype == 20 || qtype == 21 || qtype == 23,
              "only Q5_1, Q8_0, Q4_K, Q5_K, IQ4_NL, IQ3_S, and IQ4_XS are supported");
...
torch::Tensor tiered_iq_moe_gemv_impl(
  ...
  switch (qtype) {
    case 7:
      launch_tiered<2, QK5_1, QI5_1, block_q5_1,
                    VDR_Q5_1_Q8_1_MMVQ, vec_dot_q5_1_q8_1>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, expert_ids, top_k, cols, rows, tokens, stream);
      break;
    case 8:
      ...
      break;
    case 12:
      launch_tiered<QI4_K / VDR_Q4_K_Q8_1_MMVQ, QK_K, QI4_K, block_q4_K,
                    VDR_Q4_K_Q8_1_MMVQ, vec_dot_q4_K_q8_1>(
          cold_weight, hot_weight, hot_map, cold_map, cache_weight, cache_map,
          quantized, output, expert_ids, top_k, cols, rows, tokens, stream);
      break;
    case 13:
      launch_tiered<QI5_K / VDR_Q5_K_Q8_1_MMVQ, QK_K, QI5_K, block_q5_K,
...
void tiered_iq_moe_cache_lru_prepare(
  ...
  TORCH_CHECK(cache_w13.size(0) > 0 &&
              ...
              "synchronous LRU cache supports one through thirty-two slots");
  TORCH_CHECK(hot_map.numel() == cold_map.numel() &&
                  hot_map.numel() == cache_map.numel() &&
                  hot_map.numel() == admission.numel(),
...
void tiered_iq_moe_cache_lru_prepare_many(
    torch::Tensor cold_w13, torch::Tensor cold_w2, torch::Tensor hot_map,
    torch::Tensor cold_map, torch::Tensor expert_ids, torch::Tensor cache_w13,
    torch::Tensor cache_w2, torch::Tensor cache_map, torch::Tensor cache_tags,
    torch::Tensor cache_clock, torch::Tensor admission, torch::Tensor stats,
    torch::Tensor pending, int64_t max_fills) {
  TORCH_CHECK(max_fills == 2 || max_fills == 4,
              "batched cache fills support max_fills 2 or 4");
  TORCH_CHECK(cold_w13.is_cuda() && cold_w2.is_cuda() && hot_map.is_cuda() &&
                  cold_map.is_cuda() && expert_ids.is_cuda() && cache_w13.is_cuda() &&
                  cache_w2.is_cuda() && cache_map.is_cuda() && cache_tags.is_cuda() &&
                  cache_clock.is_cuda() && admission.is_cuda() && stats.is_cuda() &&
                  pending.is_cuda(), "batched cache tensors must be GPU or UVA tensors");
  const auto device = cold_w13.device();
  TORCH_CHECK(cold_w2.device() == device && hot_map.device() == device &&
                  cold_map.device() == device && expert_ids.device() == device &&
                  cache_w13.device() == device && cache_w2.device() == device &&
                  ...
                  cache_clock.device() == device && admission.device() == device &&
                  stats.device() == device && pending.device() == device,
              "batched cache tensors must share one device");
  TORCH_CHECK(cold_w13.scalar_type() == torch::kUInt8 && cold_w2.scalar_type() == torch::kUInt8 &&
                  cache_w13.scalar_type() == torch::kUInt8 && cache_w2.scalar_type() == torch::kUInt8 &&
                  hot_map.scalar_type() == torch::kInt32 && cold_map.scalar_type() == torch::kInt32 &&
                  expert_ids.scalar_type() == torch::kInt32 && cache_map.scalar_type() == torch::kInt32 &&
                  cache_tags.scalar_type() == torch::kInt32 && cache_clock.scalar_type() == torch::kInt32 &&
                  admission.scalar_type() == torch::kInt32 && stats.scalar_type() == torch::kInt32 &&
                  pending.scalar_type() == torch::kInt32, "batched cache dtype mismatch");
  TORCH_CHECK(cold_w13.is_contiguous() && cold_w2.is_contiguous() && hot_map.is_contiguous() &&
                  cold_map.is_contiguous() && expert_ids.is_contiguous() && cache_w13.is_contiguous() &&
                  cache_w2.is_contiguous() && cache_map.is_contiguous() && cache_tags.is_contiguous() &&
                  cache_clock.is_contiguous() && admission.is_contiguous() && stats.is_contiguous() &&
                  pending.is_contiguous(), "batched cache tensors must be contiguous");
  TORCH_CHECK(cold_w13.dim() == 3 && cold_w2.dim() == 3 && cache_w13.dim() == 3 && cache_w2.dim() == 3 &&
                  cold_w13.size(0) == cold_w2.size(0) && cache_w13.size(0) == cache_w2.size(0) &&
                  cache_w13.size(0) == cache_tags.numel() &&
                  cache_w13.size(1) == cold_w13.size(1) && cache_w13.size(2) == cold_w13.size(2) &&
                  cache_w2.size(1) == cold_w2.size(1) && cache_w2.size(2) == cold_w2.size(2) &&
                  cache_w13.size(0) <= kMaxLogicalCacheSlots && cache_w13.size(0) > 0 &&
                  hot_map.numel() == cold_map.numel() && hot_map.numel() == cache_map.numel() &&
                  hot_map.numel() == admission.numel() && cache_clock.numel() >= cache_tags.numel() + 1 &&
                  stats.numel() >= 9 && pending.numel() >= 7 * max_fills,
              "batched cache shapes or state capacity invalid");
  const int routes = expert_ids.numel();
  if (routes <= 0 || routes > 64) return;
  c10::cuda::CUDAGuard guard(device);
  const auto stream = at::cuda::getCurrentHIPStreamMasqueradingAsCUDA();
  const int64_t w13_bytes = cold_w13.size(1) * cold_w13.size(2);
  const int64_t w2_bytes = cold_w2.size(1) * cold_w2.size(2);
  plan_expert_cache_lru_fill_many<<<1, 256, 0, stream>>>(
      hot_map.data_ptr<int>(), cold_map.data_ptr<int>(), expert_ids.data_ptr<int>(),
      cache_map.data_ptr<int>(), cache_tags.data_ptr<int>(), cache_clock.data_ptr<int>(),
      stats.data_ptr<int>(), pending.data_ptr<int>(), hot_map.numel(), cold_w13.size(0),
      cache_w13.size(0), routes, max_fills);
  for (int i = 0; i < max_fills; ++i) {
    copy_planned_expert_cache<<<kCacheCopyBlocks, 256, 0, stream>>>(
        cold_w13.data_ptr<uint8_t>(), cold_w2.data_ptr<uint8_t>(), cache_w13.data_ptr<uint8_t>(),
        cache_w2.data_ptr<uint8_t>(), pending.data_ptr<int>() + 7 * i, w13_bytes, w2_bytes, 3);
    publish_expert_cache_lru<<<1, 1, 0, stream>>>(
        cache_map.data_ptr<int>(), cache_tags.data_ptr<int>(), cache_clock.data_ptr<int>(),
        admission.data_ptr<int>(), stats.data_ptr<int>(), pending.data_ptr<int>() + 7 * i,
        cache_map.numel(), cache_tags.numel());
  }
  AT_CUDA_CHECK(hipGetLastError());
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  ...
  module.def("tiered_iq_moe_cache_lru_prepare_many",
             &tiered_iq_moe_cache_lru_prepare_many,
             "Graph-safe batched synchronous LRU cold-expert cache fills (HIP)");
