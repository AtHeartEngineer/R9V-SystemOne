...
def gpu_snapshot(sys_root: Path = Path("/sys")) -> list[dict]:
    result = []
...
def pcie_snapshot(sys_root: Path = Path("/sys")) -> list[dict]:
...
class ProgressTracker:
    ...
    def observe(self, state: dict, metrics: dict, now: float) -> list[str]:
        events = []
        identity = (
            state.get("id"),
        ...
        with os.fdopen(fd, "wb") as output:
            while time.monotonic() - started < args.duration:
                ...
                record = {
                    ...
                    "events": tracker.observe(state, metrics, time.monotonic()),
                }
