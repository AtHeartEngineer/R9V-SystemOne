...
__device__ __forceinline__ void vec_dot_q6_K_q8_1_quint(
    const block_q6_K* __restrict__ weight,
    const block_q8_1* __restrict__ input0,
    const block_q8_1* __restrict__ input1,
    const block_q8_1* __restrict__ input2,
...
#pragma unroll
  for (int i = 0; i < QR6_K; ++i) {
    ...
    const int u3 = get_int_from_int8_aligned(
        input3[bq8_offset + 2 * i].qs, iqs % QI8_1);
    const int u4 = get_int_from_int8_aligned(
        input4[bq8_offset + 2 * i].qs, iqs % QI8_1);
    const float d80 = __low2float(input0[bq8_offset + 2 * i].ds);
    const float d81 = __low2float(input1[bq8_offset + 2 * i].ds);
    const float d82 = __low2float(input2[bq8_offset + 2 * i].ds);
    const float d83 = __low2float(input3[bq8_offset + 2 * i].ds);
    const float d84 = __low2float(input4[bq8_offset + 2 * i].ds);
    const int sc = scales[4 * i];
    const int vil = (vl >> (4 * i)) & 0x0F0F0F0F;
    const int vih = ((vh >> (4 * i)) << 4) & 0x30303030;
    const int vi = __vsubss4((vil | vih), 0x20202020);
    ...
    dot2 += d82 * static_cast<float>(__dp4a(vi, u2, 0) * sc);
    dot3 += d83 * static_cast<float>(__dp4a(vi, u3, 0) * sc);
    dot4 += d84 * static_cast<float>(__dp4a(vi, u4, 0) * sc);
  }
  const float d = __half2float(weight->d);
  sum0 += d * dot0;
  sum1 += d * dot1;
...
template <int waves>
__global__ void dense_mmvq_q6_reuse5(
    const block_q6_K* __restrict__ weight,
    const block_q8_1* __restrict__ input,
    bf16* __restrict__ output, int cols, int rows) {
  const int row = blockIdx.x;
  ...
  for (int block = tid / (QI6_K / VDR_Q6_K_Q8_1_MMVQ);
    ...
    const int iqs = VDR_Q6_K_Q8_1_MMVQ *
                    (tid % (QI6_K / VDR_Q6_K_Q8_1_MMVQ));
    const int q8_block = block * (QK_K / QK8_1);
    vec_dot_q6_K_q8_1_quint(
        &weight[row * blocks_per_row + block], &input[q8_block],
        &input[input_stride + q8_block],
        &input[2 * input_stride + q8_block],
        &input[3 * input_stride + q8_block],
        &input[4 * input_stride + q8_block], iqs,
...
torch::Tensor dense_gemv_rows45(torch::Tensor weight, torch::Tensor x,
                                int64_t qtype, int64_t rows) {
  TORCH_CHECK(weight.is_cuda() && x.is_cuda(), "tensors must be on a GPU");
  TORCH_CHECK(weight.device() == x.device(), "tensors must be on the same GPU");
  TORCH_CHECK(weight.scalar_type() == torch::kUInt8, "weight must be uint8");
  ...
  TORCH_CHECK(weight.is_contiguous() && x.is_contiguous(),
              "tensors must be contiguous");
  TORCH_CHECK(weight.dim() == 2, "weight must be a packed row-major matrix");
  TORCH_CHECK(x.dim() == 2 && (x.size(0) == 4 || x.size(0) == 5),
              "expected four or five verifier rows");
  TORCH_CHECK(weight.size(0) == rows, "rows must match weight.size(0)");

  const int cols = x.size(1);
  const int tokens = x.size(0);
  const bool exact_qwen38_q8_shape =
      qtype == 8 &&
      ((rows == 10240 && cols == 2560) ||
       (rows == 5120 && cols == 2560) ||
  ...
  auto quantized = torch::empty(
      {tokens, padded / 32 * 9},
      torch::TensorOptions().dtype(torch::kInt32).device(x.device()));
  auto output = torch::empty({tokens, rows}, x.options());
  const auto stream = at::cuda::getCurrentHIPStreamMasqueradingAsCUDA();
  quantize_q8_1<<<dim3((padded + 255) / 256, tokens, 1), 256, 0, stream>>>(
      reinterpret_cast<const bf16*>(x.data_ptr()),
      reinterpret_cast<block_q8_1*>(quantized.data_ptr()), cols, padded);
  const dim3 grid(rows, 1, 1);
  const dim3 block(32, 1, 1);
  ...
  auto* out = reinterpret_cast<bf16*>(output.data_ptr());
  if (qtype == 8) {
    const auto* w = reinterpret_cast<const block_q8_0*>(weight.data_ptr());
    if (tokens == 4) {
      dense_mmvq_q8_reuse4<1><<<grid, block, 0, stream>>>(w, q8x, out, cols, rows);
    } else {
      dense_mmvq_q8_reuse5<1><<<grid, block, 0, stream>>>(w, q8x, out, cols, rows);
    }
  } else {
    const auto* w = reinterpret_cast<const block_q6_K*>(weight.data_ptr());
    if (tokens == 4) {
      dense_mmvq_q6_reuse4<1><<<grid, block, 0, stream>>>(w, q8x, out, cols, rows);
    } else {
      dense_mmvq_q6_reuse5<1><<<grid, block, 0, stream>>>(w, q8x, out, cols, rows);
    }
  }
  AT_CUDA_CHECK(hipGetLastError());
  return output;
...
__device__ __forceinline__ void vec_dot_q6_K_q8_1_pair(
    const block_q6_K* __restrict__ weight,
    const block_q8_1* __restrict__ input0,
    const block_q8_1* __restrict__ input1,
    int iqs,
...
template <int waves>
__global__ void dense_mmvq_q6_reuse2(
    const block_q6_K* __restrict__ weight,
    const block_q8_1* __restrict__ input,
    bf16* __restrict__ output, int cols, int rows) {
  const int row = blockIdx.x;
...
torch::Tensor dense_gemv_rows2(torch::Tensor weight, torch::Tensor x,
                                int64_t qtype, int64_t rows) {
  TORCH_CHECK(weight.is_cuda() && x.is_cuda(), "tensors must be on a GPU");
  TORCH_CHECK(weight.device() == x.device(), "tensors must be on the same GPU");
  TORCH_CHECK(weight.scalar_type() == torch::kUInt8, "weight must be uint8");
  ...
  TORCH_CHECK(weight.is_contiguous() && x.is_contiguous(),
              "tensors must be contiguous");
  TORCH_CHECK(weight.dim() == 2, "weight must be a packed row-major matrix");
  TORCH_CHECK(x.dim() == 2 && x.size(0) == 2,
              "expected two verifier rows");
  TORCH_CHECK(weight.size(0) == rows, "rows must match weight.size(0)");
  ...
  auto* out = reinterpret_cast<bf16*>(output.data_ptr());
  if (qtype == 8) {
    dense_mmvq_q8_reuse2<1><<<grid, block, 0, stream>>>(
        reinterpret_cast<const block_q8_0*>(weight.data_ptr()), q8x, out, cols, rows);
  } else {
    dense_mmvq_q6_reuse2<1><<<grid, block, 0, stream>>>(
        reinterpret_cast<const block_q6_K*>(weight.data_ptr()), q8x, out, cols, rows);
  }
  AT_CUDA_CHECK(hipGetLastError());
  return output;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  module.def("dense_gemv_rows2", &dense_gemv_rows2, "Exact Q8/Q6 two-row reuse");
  module.def("dense_gemv_rows45", &dense_gemv_rows45, "Exact Q8/Q6 four/five-row reuse");
