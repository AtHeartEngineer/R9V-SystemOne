...
__global__ void quantize_hc45(const bf16* x,block_q8_1* y,int64_t stride) {
...
// Each packed weight load is reused for all four or five actual token rows.
template<int tokens>
__global__ __launch_bounds__(256) void hc_q8_mmq45_exact(
 ...
 __shared__ bf16 gates[tokens][4][16];
...
torch::Tensor hc_mixmmq45(torch::Tensor w,torch::Tensor x,torch::Tensor xn) {
 ...
 TORCH_CHECK(x.dim()==2&&(x.size(0)==4||x.size(0)==5)&&x.size(1)==320&&x.stride(1)==1&&(x.stride(0)==320||x.stride(0)==336),"expected BF16 [4 or 5,320]");
 TORCH_CHECK(xn.is_cuda()&&xn.device()==x.device()&&xn.scalar_type()==torch::kBFloat16&&xn.dim()==2&&xn.size(0)==x.size(0)&&xn.size(1)==10240&&xn.stride(1)==1&&xn.stride(0)==10240,"expected BF16 xn[M,10240] on matching device");
 c10::cuda::CUDAGuard guard(x.device());
 auto q=torch::empty({x.size(0),16*36},torch::TensorOptions().dtype(torch::kUInt8).device(x.device()));
 auto y=torch::empty({x.size(0),2560},x.options());
 auto stream=at::cuda::getCurrentHIPStreamMasqueradingAsCUDA();
 quantize_hc45<<<dim3(2,x.size(0)),256,0,stream>>>(reinterpret_cast<const bf16*>(x.data_ptr()),reinterpret_cast<block_q8_1*>(q.data_ptr()),x.stride(0));
 if (x.size(0)==4) {
 hc_q8_mmq45_exact<4><<<160,256,0,stream>>>(reinterpret_cast<const block_q8_0*>(w.data_ptr()),reinterpret_cast<const block_q8_1*>(q.data_ptr()),reinterpret_cast<bf16*>(y.data_ptr()),reinterpret_cast<const bf16*>(xn.data_ptr()),xn.stride(0));
 } else {
 hc_q8_mmq45_exact<5><<<160,256,0,stream>>>(reinterpret_cast<const block_q8_0*>(w.data_ptr()),reinterpret_cast<const block_q8_1*>(q.data_ptr()),reinterpret_cast<bf16*>(y.data_ptr()),reinterpret_cast<const bf16*>(xn.data_ptr()),xn.stride(0));
 }
 AT_CUDA_CHECK(hipGetLastError());return y;
}
PYBIND11_MODULE(TORCH_EXTENSION_NAME,m){m.def("hc_mixmmq45",&hc_mixmmq45);}
