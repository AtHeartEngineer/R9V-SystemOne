...
try:
    from tools.expert_budget import cost_catalog, expert_memory, validate_manifest
except ModuleNotFoundError:
    from expert_budget import cost_catalog, expert_memory, validate_manifest


def trim(source: dict, counts: list[int], source_hash: str, *, catalog=None, runtime=None) -> tuple[dict, dict]:
    ...
    before = expert_memory(source, 0, set(), catalog=catalog, runtime=runtime)
    ...
    # Preserve the exact tensor-cost identity when deriving a placement.
    result["expert_cost_catalog"] = before[0]["expert_cost_catalog"]
    result["expert_memory"] = resolved_catalog
    ...
    after = expert_memory(result, 0, set(), catalog=catalog, runtime=runtime)
    deltas = []
    for rank in range(2):
