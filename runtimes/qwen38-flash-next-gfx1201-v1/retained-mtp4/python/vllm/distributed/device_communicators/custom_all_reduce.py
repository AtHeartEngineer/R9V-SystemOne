...
class CustomAllreduce:
    ...
    def __init__(
        ...
        """
        self._r9v_small_bf16 = os.environ.get("R9V_NATIVE_CUSTOM_AR", "0") == "1"
        if self._r9v_small_bf16:
            max_size = min(max_size, 128 * 1024)
    ...
    def register_graph_buffers(self):
        ...
        if self._r9v_small_bf16 and offset:
            logger.info("R9V native AR graph registration rank=%d addresses=%d staged=True", self.rank, len(offset))
    ...
    def should_custom_ar(self, inp: torch.Tensor):
        if getattr(self, "_r9v_small_bf16", False) and (
            self.world_size != 2 or inp.dtype != torch.bfloat16
        ):
            return False
    ...
    def custom_all_reduce(self, input: torch.Tensor) -> torch.Tensor | None:
        ...
        if self._IS_CAPTURING:
            if torch.cuda.is_current_stream_capturing():
                # R9V candidate: use owned uncached staging for captured inputs too.
    ...
    def create_shared_buffer(
        ...
        # R9V ROCm7.14: direct and vLLM allocator probes on both gfx1201
        # devices reject IPC export for4864/135936 bytes, but accept139264.
        # Round backing allocation only; metadata offsets/logical limits stay.
        if os.environ.get("R9V_NATIVE_ALIGN_IPC") == "1" and current_platform.is_rocm():
            size_in_bytes = ((size_in_bytes + 4095) // 4096) * 4096
