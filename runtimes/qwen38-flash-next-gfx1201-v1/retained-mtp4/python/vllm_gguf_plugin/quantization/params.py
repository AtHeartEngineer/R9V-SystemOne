...
from .expert_partition import packed_partition_bounds
...
_TIERED_STREAM_COPY_HOOK_ATTR = "_r9v_tiered_stream_copy_hook"


@functools.cache
def _hip_host_api():
    library_path = find_loaded_library("libamdhip64")
    if library_path is None:
        raise RuntimeError("GGUF host allocation requires the loaded HIP runtime")
...
def _materialize_gguf_moe_param(
    ...
    if shard_id in {"w1", "w3"}:
        final_shape[1] *= 2
    partition = getattr(layer, "_gguf_expert_partition", None)
    if partition is None:
        final_shape[shard_dim] //= layer.moe_config.moe_parallel_config.tp_size
    else:
        _, final_shape[shard_dim] = packed_partition_bounds(
            final_shape[shard_dim], partition
        )
    if is_uninitialized:
        _materialize_parameter_data(
            param,
...
def _gguf_moe_weight_loader(
    ...
    hook = getattr(param, _TIERED_STREAM_COPY_HOOK_ATTR, None)
    if hook is not None:
        hook.before_copy(layer, param, shard_id, loaded_weight, expert_id)
    _materialize_gguf_moe_param(layer, param, loaded_weight, shard_id)
    partition = getattr(layer, "_gguf_expert_partition", None)
    if partition is not None:
        if loaded_weight.dtype != torch.uint8 or getattr(param, "is_transposed", False):
            raise ValueError("Unequal GGUF partition requires native packed byte layout")
        if loaded_weight.ndim not in (2, 3) or shard_id not in ("w1", "w2", "w3"):
            raise ValueError("Unsupported unequal GGUF expert weight")
        full_load = loaded_weight.ndim == 3
        if full_load and loaded_weight.shape[0] != layer.local_num_experts:
            raise ValueError("Fused expert load must cover every local expert")
        destination = param.data if full_load else param.data[expert_id]
        shard_dim = (1 if shard_id == "w2" else 0) + int(full_load)
        start, width = packed_partition_bounds(loaded_weight.shape[shard_dim], partition)
        source = loaded_weight.narrow(shard_dim, start, width)
        if shard_id in ("w1", "w3"):
            destination = destination.narrow(shard_dim, 0 if shard_id == "w1" else width, width)
        if destination.shape != source.shape:
            raise ValueError("Unequal expert source and destination shapes differ")
        destination.copy_(source)
        # Release the last local view of this master before compaction replaces
        # its parameter/owner references. The source is read-only GGUF storage.
        del destination
        if hook is not None:
            hook.after_copy(layer, param, shard_id, loaded_weight, expert_id)
