...
def _r9v_amdsmi_handle(physical_device_id: int):
    """Resolve the profile's explicit PCI identity without initializing HIP."""
    handles = amdsmi_get_processor_handles()
    expected = os.environ.get("R9V_EXPECTED_GPU_BDFS", "")
    if not expected:
        return handles[physical_device_id]
    ...
    physical = (
        visible.split(",")
        if visible
        else [str(i) for i in range(len(expected.split(",")))]
    )
    if str(physical_device_id) not in physical:
        raise RuntimeError("Requested GPU is outside the R9V PCI identity lock")
    address = expected.split(",")[physical.index(str(physical_device_id))].lower()
    matches = [
        handle
        for handle in handles
        if amdsmi_get_gpu_device_bdf(handle).lower() == address
    ]
    if len(matches) != 1:
        raise RuntimeError(f"Cannot resolve unique AMD-SMI handle for {address}")
    return matches[0]
...
def _query_gcn_arch_from_amdsmi() -> str:
    ...
    handles = amdsmi_get_processor_handles()
    if os.environ.get("R9V_EXPECTED_GPU_BDFS"):
        first = int(os.environ.get("HIP_VISIBLE_DEVICES", "0").split(",")[0])
        handles = [_r9v_amdsmi_handle(first)]
    if handles:
...
def _query_total_memory_from_amdsmi(physical_device_id: int) -> int:
    ...
    handle = _r9v_amdsmi_handle(physical_device_id)
...
class RocmPlatform(Platform):
    ...
    def is_fully_connected(cls, physical_device_ids: list[int]) -> bool:
        ...
        """
        handles = [_r9v_amdsmi_handle(i) for i in physical_device_ids]
    ...
    def get_device_uuid(cls, device_id: int = 0) -> str:
        try:
            device = _r9v_amdsmi_handle(cls.device_id_to_physical_device_id(device_id))
        except AmdSmiException as error:
    ...
    def use_custom_allreduce(cls) -> bool:
        # Experimental gfx1201 TP2 exact BF16 reduction. Thin distributed
        # parity/replay check is required before setting this opt-in flag.
        if os.environ.get("R9V_NATIVE_CUSTOM_AR", "0") == "1" and "gfx1201" in _GCN_ARCH:
            ...
            for device_id in range(cls.device_count()):
                ...
                try:
                    numa_node = amdsmi_topo_get_numa_node_number(
                        _r9v_amdsmi_handle(physical_device_id)
                    )
