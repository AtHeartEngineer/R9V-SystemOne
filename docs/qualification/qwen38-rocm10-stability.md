...
Status: qualification in progress, 2026-09-08. The complete ROCm 10 candidate built successfully, including vLLM, the GGUF
plugin, and all three custom HIP extensions. No ROCm 10 serving image is yet
qualified. The release candidate remains on its existing
...
The inspected AMD wheel reports `torch==2.11.0+rocm10.0.0` and
`torch.version.hip == 7.15.26333`; the ROCm distribution version is `10.0.0`.
ROCm and HIP version numbers are separate. The first GPU gate also found that
installing the base image's AMD SMI Python bindings loaded a second AMD SMI
library alongside the wheel's copy. With torch imported, AMD SMI returned zero
devices even though HIP could allocate tensors. Use the bindings shipped inside
`_rocm_sdk_core/share/amd_smi` with their matching library. The corrected probe
saw one selected R9700 and one mapped AMD SMI library. QSA scoring against an
independent FP32 reference and fused GDN eager/state/graph/FLA parity gates passed
on the candidate; these are kernel checks, not whole-model qualification.

The first serving attempt passed worker BDF/UVA/H2D/collective probes and loaded
the model, then failed when `torch_shm_manager` could not resolve the SDK's
OpenBLAS/HIP libraries. Python import alone does not test helper executables.
The Dockerfile now registers the SDK library directories with the dynamic
loader and requires a bounded `file_system` shared-memory allocation to pass
at build time. The small reproduction passes with those paths configured. Resolve framework and application
dependencies together: the fork's ROCm requirements need setuptools below 80,
and its Numba/scientific dependencies cannot use the initially selected NumPy
2.5. The joint candidate lock is in the runtime's `rocm10/application.txt`.
...
| Bounded workload | Separate sessions of at most 45 minutes, including startup and cleanup | Text/tools/vision, maximum advertised context, per-card headroom, host-memory peaks, errors and preserved failure evidence |
| Repeatability and restart | Separate bounded comparison and restart sessions | Frozen warmup/workload, baseline–candidate–baseline, no unplanned restarts, per-image/configuration evidence |
| Delayed failures | Passive rolling telemetry during normal use; additional user-host reports | Crash bundles and memory/latency trends. Bounded tests do not establish hours-long stability; an eight-hour local soak is not a release requirement |
